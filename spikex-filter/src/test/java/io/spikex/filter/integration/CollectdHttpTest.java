 ...
 * @author cli
 */
public class CollectdHttpTest extends TestVerticle implements Handler<Message<JsonObject>> {
    ...
    private static final String FILTER_SRC_ADDRESS = "collectd-data-src";
    private static final String FILTER_DEST_ADDRESS = "collectd-data-dest";

    private static final Logger m_logger = LoggerFactory.getLogger(CollectdHttpTest.class);
    ...
    public void testHttpWriter() throws Exception {
        ...
        if (HostOs.isMac() || HostOs.isLinux()) {
            ...
            // Start collectd
            new CollectdThread("collectd-thread", "collectd-http.conf").start();

            // Http server configuration
            JsonObject config = createBaseConfig();
            config.putArray("add-tags", new JsonArray(new String[]{"collectd"}));
            config.putString("input-format", "collectd-json");
